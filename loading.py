...
def load_data(db_url, datetime_dim_data, customer_dim_data, product_dim_data, fact_table_data):
    ...
    try:
        ...
        for index, row in product_dim_data.iterrows():
            product_dim_object = ProductDim(stock_code=row['stock_code'], description=row['description'], unit_price=row['unit_price'])
        ...
        fact_table_objects = [FactTable(invoice_no=row['invoice_no'], stock_code=row['stock_code'], quantity=row['quantity'], datetime_id=row['datetime_id'], customer_id=row['customer_id']) for index, row in fact_table_data.iterrows()]
