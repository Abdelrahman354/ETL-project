...
def transform_data(df):
    ...
    product_dim_data = df[['StockCode', 'Description', 'UnitPrice']].drop_duplicates().rename(columns={'StockCode': 'stock_code', 'Description': 'description', 'UnitPrice': 'unit_price'})
    
    fact_table_data = df[['InvoiceNo', 'StockCode', 'Quantity', 'InvoiceDate_Date', 'InvoiceDate_Time', 'CustomerID']].rename(columns={'InvoiceNo': 'invoice_no', 'StockCode': 'stock_code', 'Quantity': 'quantity', 'CustomerID': 'customer_id'})
